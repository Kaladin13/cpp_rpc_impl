  ...
  private:
    json HandleSingleRequest(json &request) {
      ...
      try {
        ...
        return json{{"id", id}, {"error", {{"code", internal_error}, {"message", std::string("internal rpcServer error: ") + e.what()}}}, {"jsonrpc", "2.0"}};
      } catch (...) {
        return json{{"id", id}, {"error", {{"code", internal_error}, {"message", std::string("internal rpcServer error")}}}, {"jsonrpc", "2.0"}};
      }
    }
