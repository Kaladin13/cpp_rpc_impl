  ...
  private:
    ...
    JsonRpcResponse call_method(const id_type &id, const std::string &name, const json &params) {
      ...
      try {
        ...
        throw JsonRpcException(internal_error, R"(invalid rpcServer response: neither "result" nor "error" fields found)");
      } catch (json::parse_error &e) {
        throw JsonRpcException(parse_error, std::string("invalid JSON response from rpcServer: ") + e.what());
      }
    }
